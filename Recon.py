...
from ReconGUI05 import Ui_ReconGUI  # GUI module
...
#         self.msgPrint("image size" + str(d1Mg[0].shape) + "; image3D size" + str(self.image3D.shape)+ os.linesep)
    ...
    def writeDicomFiles12 (self):
        ...
        if not fileName:  #if cancel is pressed return
            return None
        self.dataSet[0].writeDicomFiles(fileName + "kmag")   #write current image list in DICOM format to filename+ imagenumber + .dcm
        self.dataSet[1].writeDicomFiles(fileName + "kphase")   #write current image list in DICOM format to filename+ imagenumber + .dcm
        
    def writeDicomFiles34 (self):
        fileName = QtGui.QFileDialog.getSaveFileName(parent=None, caption="Dicom File Name")
        if not fileName:  #if cancel is pressed return
            return None
        self.dataSet[2].writeDicomFiles(fileName + "rmag")   #write current image list in DICOM format to filename+ imagenumber + .dcm
        self.dataSet[3].writeDicomFiles(fileName + "rphase")   #write current image list in DICOM format to filename+ imagenumber + .dcm
